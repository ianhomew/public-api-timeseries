...
- **`mof_press`（財政部）**：部分新聞稿的聯絡人簽名檔含**承辦人姓名與個人行動電話**，
  重複出現於 4 篇以上。另有一起**機關事後靜默刪除個資**的案例：2026-08-29 偵測到一篇
  既有公告（臺北關通關服務宣導活動通知）正文中的聯絡人姓名、市話與行動電話整行被財政部
  自行刪除、未另行公告，本專案依既有機制如實記錄改寫前後 diff 於
  `changes/mof_press/2026-08-29.md`（commit `9122f81`，已推送公開）。**使用者裁示
  （Y6，2026-09-08）：不遮蔽**，延續 `moea_press` 的既有決策原則（「原本就是公開的，
  維持現況」）——這行聯絡資訊是財政部自行公開刊登於官網新聞稿正文，本專案保存的是機關
  發布當下的原貌，以及「事後被靜默刪除」這個事實本身；若因機關事後刪除就回溯遮蔽已存檔
  的原文，會讓本節開頭所述「內容是否被改寫的比對基準」失真，也會與 `moea_press` 現行
  仍在架上之個資的處理原則不一致。
