"""金管會 法規草案預告（id=133，與現有 fsc_clarification 同網域同一套 CMS）。

清單：https://www.fsc.gov.tw/ch/home.jsp?id=133&parentpath=0,3&mcustomize=lawnotice_list.jsp&page=<N>
內頁：https://www.fsc.gov.tw/ch/home.jsp?id=133&parentpath=0,3&mcustomize=lawnotice_view.jsp&dataserno=<12碼>&dtable=NoticeLaw

正文容器與 fsc_clarification 相同（class="ap" → 切到 <!--ap END -->），本站已各自重新實測（2026-08-28）。
...
KEY = "fsc_lawnotice"
DESC = "金管會法規草案預告"
SOURCE_HOME = "https://www.fsc.gov.tw/ch/home.jsp?id=133&parentpath=0,3"
...
CH = {"id": "133", "parentpath": "0,3",
      "list_mc": "lawnotice_list.jsp", "view_mc": "lawnotice_view.jsp", "dtable": "NoticeLaw"}
MAX_PAGES = 8          # 每年約38.5筆，最新100筆以內；每頁15筆，8頁=120筆已足夠覆蓋
PARSER_VERSION = 1
...
def collect(fetch, clean, deadline=None):
    ...
    sernos = sernos[:100]
    if not sernos:
        raise RuntimeError("fsc_lawnotice 清單 0 筆 —— 視為抓取失敗")
    items = []
    for s in sernos:
        if _deadline_hit(deadline):
            break
        ...
        time.sleep(1)
    if not items:
        raise RuntimeError("fsc_lawnotice 內頁全部解析失敗（可能改版）")
    return items
