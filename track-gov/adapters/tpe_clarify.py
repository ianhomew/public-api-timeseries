...
MIN_BODY_LEN = 30      # R3（v5）：覆寫共用層預設 50。本 adapter 內容本來就精簡，原本內部
                        # 已用 30 這個門檻（見下方 collect() 內 `len(body) < 30`），沿用同一
                        # 數字宣告給共用層，避免共用層預設 50 誤傷本站原本就允許的較短正文
                        # （實測本站既有快照最短真實正文為 58 字，仍在 30 之上，兩層一致不衝突）。
