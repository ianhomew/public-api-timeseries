...
TIME_BUDGET_SECS = 3000  # 【第二輪覆蓋率調查修正,2026-08-31】原 600 秒會在第 72~84 頁被腰斬（實測），
# 165 頁連續實測平均延遲 7.19s/頁（比單頁孤立測試 3.55s 高，需用連續抓取的真實延遲估算），
# 165*(7.19+1)≈1351s（平均情境）、165*(12.19+1)≈2176s（觀測到的最差單頁延遲情境）。
# 3000 秒同時涵蓋兩者並留安全餘裕；經驗證不會與 09:30 track-gov 的 flock -w 1800 排程相撞
# （詳見 docs/virtuals-coverage-plan.md）。超過即停止翻頁並標記 truncated
